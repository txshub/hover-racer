...
public class Barriers {
	 ...
	 * 
	 * @param track List of the points on the track, as supplied for the rendering engine. Even indexes are points on the left of the track
	 *        and odd are points on the right. */
	...
	/** Creates all lines a supplied ship collides with (there might occasionally be more than one at a time).
	 * 
	 * @param ship Ship to find the collisions for. Only its position and size are used.
	 * @return Lines describing walls to collide with. They are all 2d direction vectors, and the x and z components of the velocity vector
	 *         should be reflected on them for collisions. */
	...
	/** Returns all collision vectors for a specific point on the track (both left and right walls from that point). For details see
	 * {@link allCollisions}. */
	...
	/** Returns all collision vectors for a specific wall on the track. For details see {@link allCollisions}. */
	private Optional<ImVector2f> collisionVector(ImVector2f pos, ImVector2f start, ImVector2f end, float size) {
		if (size < distanceTo(pos, start, end)) return Optional.empty();
		else return Optional.of(end.sub(start));
	}

	/** Finds the distance from a point to a given wall
	 * 
	 * @param pos Position of a ship
	 * @param start Point where the wall starts
	 * @param end Point where the wall ends
	...
	/** @param i Index of the point. Values outside the bounds are cast to proper indexes: -1 becomes the last point etc.
