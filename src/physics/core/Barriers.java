...
public class Barriers {
	...
	public Barriers(List<TrackPoint> track) {
		...
		// Generate barriers
		for (int i = 0; i < points; i++) {
			ImVector2f side = new ImVector2f(middle.get(i).getY(), -middle.get(i).getX()).normalize().mul(track.get(i).getWidth() / 2);
	...
	private Optional<ImVector2f> collisionVector(ImVector2f pos, ImVector2f start, ImVector2f end, float size) {
		if (size < distanceTo(pos, start, end, false)) return Optional.empty();
	...
	private float distanceTo(ImVector2f pos, ImVector2f start, ImVector2f end, boolean printit) {
		...
		/* float t = Math.max(0f, Math.min(1f, new ImVector2f(end).sub(pos).dot(new ImVector2f(start).sub(pos)) / dist2));
		 * ImVector2f projection = new ImVector2f(end).sub(new ImVector2f(start).mul(t));
		 * if (printit) System.out.println("Projection at " + t + ":" + projection + ", between " + start + " and " + end); */
