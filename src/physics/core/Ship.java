...
public abstract class Ship extends Entity {
  ...
  private void airResistance(float delta) {
    velocity.forEach(v -> Math.signum(v)
        * Math.max(0, (Math.abs(v) - delta * Math.sqrt(Math.abs(v) * stats.AIR_RESISTANCE))));
  }
  ...
  protected void steer(float thrust, float breaking, float turn, float strafe, float delta) {
    ...
    if (turn != 0) {
      ...
      rotationalVelocity
          .changeZ(z -> z + delta * turn * stats.TURN_SPEED * SPEED_OF_ROTATION_WHILE_TURNING);
    }
    // Accelerating
    ...
    if (strafe != 0) {
      ...
      rotationalVelocity
          .changeZ(z -> z + delta * strafe * stats.TURN_SPEED * SPEED_OF_ROTATION_WHILE_STRAFING);
    }
  }
