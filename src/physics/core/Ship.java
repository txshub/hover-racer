...
public abstract class Ship extends Entity {
  ...
  private void airResistance(float delta) {
    velocity.forEach(v -> Math.signum(v)
        * Math.max(0, (Math.abs(v) - delta * Math.sqrt(Math.abs(v) * AIR_RESISTANCE))));
  }
  ...
  private void airCushion(float delta) {
    ...
    if (distance <= 0 && velocity.getY() < 0) {
      ...
      velocity.changeY(y -> y
          + delta * AIR_CUSHION * VERTICAL_SCALE / Math.pow(Math.max(distance, 1), CUSHION_SCALE));
  }
  ...
  protected void steer(float thrust, float breaking, float turn, float strafe, float jump,
      float brokenDelta) {
    if (!started)
    ...
    if (turn != 0) {
      ...
      rotationalVelocity
          .changeZ(z -> z + delta * turn * TURN_SPEED * SPEED_OF_ROTATION_WHILE_TURNING);
    }
    // Accelerating
  ...
  private void shipCollisions() {
    otherShips.stream().filter(
        ship -> ship.getInternalPosition().distanceTo(this.position) <= ship.getSize() + this.size)
  ...
  public String toString() {
    return "Ship No " + id + " at " + position + " facing " + rotation + "\nLinear velocity is "
        + velocity + " and angular velocity is" + rotationalVelocity;
  }
