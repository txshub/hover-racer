...
public abstract class Ship extends Entity {
  ...
  protected void steer(float thrust, float turn, float strafe, float jump, float brokenDelta) {
    ...
    if (!ACTUALLY_BREAK && thrust != 0)
      accelerate2d(delta * thrust * ACCELERATION, (float) Math.PI * 1.5f);
    else if (ACTUALLY_BREAK && thrust > 0)
      accelerate2d(delta * thrust * ACCELERATION, (float) Math.PI * 1.5f);
    else if (ACTUALLY_BREAK && thrust < 0)
      airResistance(-delta * thrust * BREAK_POWER);
    // Strafing
    if (strafe != 0)
      accelerate2d(delta * strafe * ACCELERATION, (float) Math.PI); // TODO
                                                                    // change
                                                                    // strafing
                                                                    // to an
                                                                    // instant
