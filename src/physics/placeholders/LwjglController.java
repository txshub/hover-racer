...
public class LwjglController implements ControllerInt {
  ...
  HashSet<Action> pressed; // Set of currently pressed keys (updated with each
                           // call of getPressedKeys)
  HashMap<Integer, Action> mapping; // Maps key codes to actions (can be changed
                                    // via changeKey)
  ...
  private void updateKey(Integer keyCode) {
    if (Keyboard.isKeyDown(keyCode))
      pressed.add(mapping.get(keyCode));
    else
      pressed.remove(mapping.get(keyCode));
  }
