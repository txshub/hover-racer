...
public class Visualisation {
  ...
  public void draw(Graphics g) {
    ...
    for (Ship s : ships) {
      ...
      float z = -s.getPosition().z + 120; // Use this line to get a top-down
                                          // view
      // float z = (500 - s.getPosition().y * 2); // Use this line to get a
      // sideways view
      Rectangle r = new Rectangle((int) (x - shipWidth / 2) * scale,
          (int) (z - shipLength / 2) * scale, shipWidth * scale, shipLength * scale);

      AffineTransform trans = g2.getTransform();
      //
      g2.setColor(Color.white);
      g2.rotate(-Math.toRadians(s.getRotation().y), r.getCenterX(), r.getCenterY());
      g2.fillRect((int) (x - shipWidth / 2) * scale, (int) (z - shipLength / 2) * scale,
          shipWidth * scale, 10);
      g2.draw(r);
      g2.fillRect((int) (x - shipWidth / 2) * scale, (int) (z - shipLength / 2) * scale,
          shipWidth * scale, 10);
      ...
      // g2.drawString("ax: " + df.format(s.getAcl().x) + " ay: " +
      // df.format(s.getAcl().y) + " vx: " + df.format(s.getVel().x) + "
      // vy: "
      // + df.format(s.getVel().y) + " px: " + df.format(s.getPos().x) + " py: "
      // + df.format(s.getPos().y), 10, 10);
    }
  }
