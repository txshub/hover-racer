...
public class MultiplayerShipManager implements ServerShipProvider {
	...
	public MultiplayerShipManager(byte playerId, InputController input, List<TexturedModel> models,
		...
		for (byte id = 0; id < models.size(); id++) {
			...
			RemoteShip remoteShip = new RemoteShip(id, models.get(id), startingPositions.get(id), ground, stats.get(id),
					track, this);
		...
		this.player = new PlayerShip(playerId, models.get(playerId), startingPositions.get(playerId), remotes, ground,
				stats.get(playerId), track, input);
	...
	public MultiplayerShipManager(byte playerId, InputController input, TexturedModel playerTexture,
		...
		remotes.forEach(r -> r.addOtherShips(remotes));
		// Finally create the player
		this.player = new PlayerShip(playerId, playerTexture, startingPositions.get(playerId), remotes, ground, null,
				null, input);
		remotes.forEach(r -> r.addOtherShip(player));
	}
