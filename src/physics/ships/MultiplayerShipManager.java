...
public class MultiplayerShipManager implements ServerShipProvider {
  ...
  private MultiplayerShipManager(byte playerId, InputController input, List<TexturedModel> models,
    ...
    for (byte id = 0; id < models.size(); id++) {
      if (id == playerId)
        continue; // Don't give them player's id
    ...
    try {
      ...
      System.err.println("Failed to create sounds");
    }
  ...
  /**
   * Add a new position packet received from the server. This will update all
   * remote ships.
   * 
   * @param packet
  ...
  public void addPacket(byte[] packet) {
    int j = 0;
    if (packet.length % SHIP_PACKET_LENGTH != 0)
  ...
  /**
   * @return The position packet for the player, to be sent to the server as a
   *         message of type SENDPLAYERDATA
   */
