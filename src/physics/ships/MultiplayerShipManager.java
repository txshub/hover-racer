...
/**
 * A class that manages ship on client side while in multiplayer mode. Handles creating them,
 * updating and communicating with the server.
 * 
...
public class MultiplayerShipManager implements ServerShipProvider {
   ...
   * 
   * @param data
   *          Data received from the server during setup
   * @param ground
   *          Ground for ships to levitate over
   * @param input
   *          Input for the player
   * @param loader
   *          Loader object for loading textures
   * @param track
   *          Track to generate the barriers from
   */
  public MultiplayerShipManager(RaceSetupData data, GroundProvider ground, InputController input,
      Loader loader, ArrayList<TrackPoint> track) {
    this(data.getYourId(), input, makeModels(data.getModels(), data.getTextures(), loader),
        data.getStartingPositions(), data.getStartingOrientation(), data.getStats(), ground, track);
  }

  private MultiplayerShipManager(byte playerId, InputController input, List<TexturedModel> models,
    ...
    for (byte id = 0; id < models.size(); id++) {
      ...
      RemoteShip remoteShip = new RemoteShip(id, models.get(id), startingPositions.get(id), ground,
          stats.get(id), track, this);
    ...
    this.player = new PlayerShip(playerId, models.get(playerId), startingPositions.get(playerId),
        remotes, ground, stats.get(playerId), track, input);
  ...
  public MultiplayerShipManager(byte playerId, InputController input, TexturedModel playerTexture,
    ...
    for (TexturedModel texturedModel : otherTextures) {
      ...
      remotes.add(
          new RemoteShip(id, texturedModel, startingPositions.get(id++), ground, null, null, this));
    }
    // Tell ships about each other
    remotes.forEach(r -> r.addOtherShips(remotes));
    // Finally create the player
    this.player = new PlayerShip(playerId, playerTexture, startingPositions.get(playerId), remotes,
        ground, null, null, input);
    remotes.forEach(r -> r.addOtherShip(player));
  }
  ...
  /**
   * Starts the race, allowing input through. This is when racing actually starts, so probably a few
   * seconds after the game is loaded.
   */
  ...
  /**
   * Adds a client-side AI to a multiplayer game, only to be used for testing. The server won't know
   * about it, but it will collide with things. Call before addShipsTo(entities) or it won't be
   * rendered.
   * 
   * @param aiShip
   ...
   * 
   * @param packet
   *          Packet from message of type FULLPOSITIONUPDATE
   */
  ...
  /**
   * @return The position packet for the player, to be sent to the server as a message of type
   *         SENDPLAYERDATA
   */
  ...
  private static TexturedModel makeModel(String model, String texture, Loader loader) {
    TexturedModel model1 = new TexturedModel(OBJFileLoader.loadOBJ(model, loader),
