...
public class AIShip extends Ship {
  ...
  public void update(float delta) {
    ...
    if (Keyboard.isKeyDown(Keyboard.KEY_I) || input == null) {
      ...
      // if (segAngle <= piOverEight) thrust = (float) (-(4/Math.PI) * segAngle
      // + 1);
      ...
      if (input.checkAction(Action.FORWARD))
        thrust++;
      if (input.checkAction(Action.BREAK))
        thrust--;
      if (input.checkAction(Action.TURN_RIGHT))
        turn++;
      if (input.checkAction(Action.TURN_LEFT))
        turn--;
      if (input.checkAction(Action.STRAFE_RIGHT))
        strafe++;
      if (input.checkAction(Action.STRAFE_LEFT))
        strafe--;
      if (input.checkAction(Action.JUMP))
        jump++;
    }
