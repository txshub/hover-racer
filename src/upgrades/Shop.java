...
public class Shop {
  ...
  public Collection<UpgradeDisplayData> getUpgradeData() {
    ...
    for (UpgradeType upgrade : allUpgrades) {
      if (ship.getUpgrades().containsKey(upgrade)) {
        res.add(new UpgradeDisplayData(upgrade.name, ship.getCost(upgrade),
            ship.getUpgrades().get(upgrade), upgrade.maxLevel, convertStats(upgrade.stats)));
      } else {
        res.add(new UpgradeDisplayData(upgrade.name, ship.getCost(upgrade), 0, upgrade.maxLevel,
  ...
  public int buy(String name) {
    UpgradeType upgrade = allUpgrades.stream().filter(u -> u.name.equals(name)).findAny().get(); // Get
                                                                                                 // the
                                                                                                 // upgrade
                                                                                                 // with
                                                                                                 // this
                                                                                                 // name
    if (ship.getCost(upgrade) > money)
      throw new IllegalStateException("Tried buying an upgrade without sufficient funds");
