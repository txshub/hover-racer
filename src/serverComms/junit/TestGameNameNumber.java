...
public class TestGameNameNumber {
	...
	public void testToStringWithInt() {
		...
		if(!gnn.toString().equals(expected)) fail("toString with int failed");
	}
	...
	public void testToStringNoInt() {
		...
		if(!gnn.toString().equals(construct)) fail("toString no int failed");
	}
	...
	public void testToByteArrayWithInt() {
		...
		GameNameNumber gnn = new GameNameNumber(name,num);
		if(!Arrays.equals(gnn.toByteArray(), expected)) fail("Byte arrays don't match with int");
	}
