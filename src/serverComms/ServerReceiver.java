			...
			while (true) {
				...
				if (fullMsg.getType() == ServerComm.BADPACKET) {
					...
					lobby.clientTable.getQueue(clientName).offer(new ByteArrayByte(
						lobby.clientTable.getGame(lobby.clientTable.getGameID(clientName)).toByteArray(), ServerComm.VALIDGAME));
					...
					if (!lobby.clientTable.joinGame(data.id, data.data)) {
						...
						lobby.clientTable.getQueue(clientName)
							.offer(new ByteArrayByte(lobby.clientTable.getGame(data.id).toByteArray(), ServerComm.VALIDGAME));
					}
				} else if (fullMsg.getType() == ServerComm.SENDPLAYERDATA) {
					// System.out.println("Send Data"); //TODO no prints in dev pls
					if (gameRoom != null) {
					...
					if (gameID == -1) {
						lobby.clientTable.getQueue(clientName)
							.offer(new ByteArrayByte(("Can't start invalid game").getBytes(ServerComm.charset), ServerComm.INVALIDGAME));
					} else { // Valid Game
					...
					if (gameID == -1) {
						...
						lobby.clientTable.getQueue(clientName)
							.offer(new ByteArrayByte(lobby.clientTable.getGame(gameID).toByteArray(), ServerComm.VALIDGAME));
					}
				} else {
