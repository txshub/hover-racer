...
public class DetectTimeout extends Thread {
   ...
   * @param table The ClientTable to remove the user from if they disconnect
  ...
  @Override
  /**
   * Runs the DetectTimeout object
   * (Called with DetectTimeout.start())
   */
  public void run() {
    try {
      ...
      if (!messageReceived) { //If no message is received
        if (ServerComm.DEBUG)
          System.out.println("Client " + user + " disconnected"); //If debug is on, print a disconnect message
