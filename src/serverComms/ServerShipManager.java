...
public class ServerShipManager implements ServerShipProvider {
  ...
  public ServerShipManager(RaceSetupData data, int players, int ais) {
    ...
    if (amount != players + ais)
      throw new IllegalStateException("Mismatch between ShipSetupData and amount of players");
    ships = new ArrayList<Ship>(amount);
    for (int i = 0; i < amount; i++) {
      if (i <= players)
        ships.add(new RemoteShip((byte) i, null, data.getStartingPositions().get(i), ground, this));
      else
        ships.add(
            new AIShip((byte) i, null, data.getStartingPositions().get(i), null, ground, null));
    }
