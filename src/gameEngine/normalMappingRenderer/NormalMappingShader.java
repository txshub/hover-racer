...
public class NormalMappingShader extends ShaderProgram {
  ...
  protected void getAllUniformLocations() {
    ...
    for (int i = 0; i < MAX_LIGHTS; i++) {
      location_lightPositionEyeSpace[i] = super.getUniformLocation(
          "lightPositionEyeSpace[" + i + "]");
  ...
  protected void loadLights(List<Light> lights, Matrix4f viewMatrix) {
    for (int i = 0; i < MAX_LIGHTS; i++) {
      if (i < lights.size()) {
        super.loadVector(location_lightPositionEyeSpace[i],
            getEyeSpacePosition(lights.get(i), viewMatrix));
